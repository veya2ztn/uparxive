...
def put_ref_back_and_clean_format(sentense,labels_reference,paper_id, refs_that_wont_recovery, config):
    ns = {'default': 'http://dlmf.nist.gov/LaTeXML'}
    for ref in sentense.xpath('.//default:ref', namespaces=ns):recovery_ref_in_sentense(ref, labels_reference,paper_id,config)
    ...
    for ref in sentense.xpath('.//default:cite', namespaces=ns):recovery_citation_in_sentense(ref, 
                                                                                              labels_reference,
                                                                                 paper_id,
                                                                                 refs_that_wont_recovery,
                                                                                 config)
    return sentense
...
def discard_note(tree, filte_out_note):
    ns = {'default': 'http://dlmf.nist.gov/LaTeXML'}
    name = 'note'
    # Find all 'Math' elements using the namespace
    note_ref_metadata= {}
    ...
    for note_id, element in enumerate(elements):
        ...
        if filte_out_note:
            ...
            note_ref_metadata[f"Note_{note_id}"]=[True, deepcopy(element)]
...
def format_ref(ref_type, label,paper_id, config:XMLtoJsonConfig):
    ...
    return label
### deal with math
def recovery_ref_in_sentense(ref, labels_reference,paper_id, config:XMLtoJsonConfig):
    element = ref.getparent()
...
def recovery_citation_in_sentense(cite, labels_reference,paper_id,refs_that_wont_recovery,config:XMLtoJsonConfig):
    
    ns = {'default': 'http://dlmf.nist.gov/LaTeXML'}
    bibref = cite.find('.//default:bibref', ns)
    if bibref is None:
        checkCiteDontHaveBibRef(f""" cite of  {view_xml(cite)}  dont have bibref??? """)
        cite.getparent().remove(cite)
        return 
    ...
    if not string.strip():
        ...
        if left:
            ...
            left = ""
        label = format_the_smart_citation(left, label, right, paper_id,use_plain_citation=config.use_plain_citation) 
...
def cleanup_xml(tree, whole_ref_to_labels,paper_id,refs_that_wont_recovery, config):
    tree = revert_math_into_its_origin_text_in_xml(tree)
    tree = remove_MathFork_in_equation(tree)
    put_ref_back_and_clean_format(tree,whole_ref_to_labels,paper_id,refs_that_wont_recovery,config)
    tree = beauty_each_sentense(tree)
    tree = replace_equation_blocks(tree)
    tree = replace_item_block_with_markdown_format(tree)
    return tree

def cleanup_reference_string(string, whole_ref_to_labels,paper_id, refs_that_wont_recovery, config):
    string= cleanup_xml(string, whole_ref_to_labels, paper_id, refs_that_wont_recovery, config)
...
import json
import io
def deal_with_xml_file(tmp_xml_path, output_dir,  config:XMLtoJsonConfig):
    reterive_result_mode = config.reterive_result_mode
    verbose              = config.verbose
    filte_out_note    = not config.passNote
    use_count_type_ref= not config.use_origin_ref_number
    use_smart_citation= not config.use_plain_citation
    ...
    note_ref_metadata = discard_note(tree, filte_out_note)
    ...
    for remain_key, remain_val in note_ref_metadata.items():
        ...
        string = cleanup_reference_string(remain_val[1], whole_ref_to_labels,paper_id, put_back_keys, config)
    ...
    tree = cleanup_xml(tree, whole_ref_to_labels,paper_id, put_back_keys, config)
    ...
    for remain_key, remain_val in figures_metadata.items():
        string = cleanup_reference_string(remain_val, whole_ref_to_labels,paper_id, put_back_keys, config)
    ...
    xml_memory_file = io.BytesIO()
    tree.write(xml_memory_file, pretty_print=True, xml_declaration=True, encoding='UTF-8')
    xml_memory_file.seek(0)
    soup = BeautifulSoup(xml_memory_file, 'xml')
    with open("/home/zhangtianning.di/projects/unique_data_build/bs4xml.xml", 'w', encoding='UTF-8') as file:
...
def xml_to_json_one_path(file_path, args:XMLtoJsonConfig)->Tuple[str,str]:
    ...
    if os.path.exists(output_path) and not args.redo:
        return arxivid, 'Skip'
    try:
        deal_with_xml_file(file_path, output_dir,args)
