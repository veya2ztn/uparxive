...
from uparxive.lougat.collect_data_into_parquet import process_one_file_wrapper, ParquetConverterConfig
...
if __name__ == '__main__':
    ...
    parser.add_arguments(ParquetConverterConfig, dest="config")
    ...
    for arxivid, _type in results:
        if _type not in analysis:
            analysis[_type] = []
        if isinstance(arxivid,list):
            analysis[_type].extend(arxivid)
        else:
